...
export class EstructuraAcademicaService {
  ...
  // ---------- AULAS ----------
  listarAulas(soloActivos = false): Observable<any> {
    return this.http.get(`${this.apiUrl}/aulas?soloActivos=${soloActivos}`, { headers: this.headers() });
  }
  crearAula(data: any): Observable<any> {
    return this.http.post(`${this.apiUrl}/aulas`, data, { headers: this.headers() });
  }
  editarAula(id: string, data: any): Observable<any> {
    return this.http.patch(`${this.apiUrl}/aulas/${id}`, data, { headers: this.headers() });
  }
  eliminarAula(id: string): Observable<any> {
    return this.http.delete(`${this.apiUrl}/aulas/${id}`, { headers: this.headers() });
  }
