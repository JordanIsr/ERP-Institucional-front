...
private prepararEdicionMalla(
  ...
  activarMalla(): void {
    const malla = this.mallaSeleccionada;
    if (!malla || !confirm(`¿Activar la malla ${malla.codigo}? Todas sus carreras, niveles y asignaturas deben estar completas.`)) return;
    this.service.activarMallaGeneral(malla.id).subscribe({
      next: () => { this.mensaje = 'Malla activada correctamente.'; this.cargarMallas(malla.id); },
      error: (e) => this.error = this.mensajeError(e, 'No se pudo activar la malla.'),
    });
  }
...
if (periodoSeleccionado.estado === 'CERRADO') {
  ...
  guardarParalelo(): void {
    ...
    if (!datos.nombre || !/^[A-Z0-9]+(?:[ -][A-Z0-9]+)*$/.test(datos.nombre)) { this.error = 'El nombre del paralelo es obligatorio y solo admite letras, números, espacios o guiones.'; return; }
